...
<x-app-layout>
    ...
    @endphp
        ...
        <h1>Choferes disponibles para tu viaje por $<span>{{$price}}</span></h1>
        
        @foreach ($choferes as $chofer)
            <form action="{{route('pago')}}" method="POST">
                @csrf
                <input type="hidden" name="price" value="{{$price}}">
                <input type="hidden" name="nombre_chofer" value="{{$chofer->name}}">
                <h2>{{$chofer->vehicle->brand}} {{$chofer->vehicle->model}}</h2>
                <p>{{$chofer->rating}} estrellas</p>
                <p>A {{$distance}} kilometros de ti</p>
                <h3>Nombre del chofer: {{$chofer->lastname}}, {{$chofer->name}}</h3>
